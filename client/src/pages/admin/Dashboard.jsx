...
import { Users, Package, ShoppingCart, DollarSign, Clock, Calendar, Sparkles } from 'lucide-react'
    ...
    const fetchAdminOrders = async () => {
      try {
        ...
        if (res.data && Array.isArray(res.data)) {
          ...
          const totalSales = fetchedOrders.reduce((sum, ord) => sum + Number(ord.cartTotal || 0), 0)
          const totalOrders = fetchedOrders.length
          const avgOrderValue = totalOrders > 0 ? totalSales / totalOrders : 0

          const completedCount = fetchedOrders.filter(ord => ord.orderStatus === 'Completed' || ord.orderStatus === 'สำเร็จ').length
          const completedPercent = totalOrders > 0 ? Math.round((completedCount / totalOrders) * 100) : 0
    ...
    <div className="space-y-6 p-6 sm:p-8 bg-gradient-to-br from-[#FAF8F5] via-[#FFFDF9] to-[#F4EFEB] min-h-screen">
      ...
      <div className="bg-white/85 backdrop-blur-xl p-6 rounded-[2rem] border border-[#EBE5DC] shadow-[0_10px_30px_rgba(0,0,0,0.03)] flex items-center justify-between">
        <div className="space-y-1">
          ...
          <p className="text-xs text-stone-400 pl-9">ภาพรวมและสถิติคำสั่งซื้อสินค้าภายในร้านของคุณ</p>
        </div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
        ...
        <div className="bg-white/85 backdrop-blur-xl p-5 rounded-[2rem] border border-[#EBE5DC] shadow-[0_8px_20px_rgba(0,0,0,0.02)] flex items-center gap-4 hover:shadow-md transition-all">
          ...
          <div>
            ...
            <h3 className="text-xl font-extrabold text-stone-800 mt-0.5">{stats.totalProducts}</h3>
          </div>
        </div>
        ...
        <div className="bg-white/85 backdrop-blur-xl p-5 rounded-[2rem] border border-[#EBE5DC] shadow-[0_8px_20px_rgba(0,0,0,0.02)] flex items-center gap-4 hover:shadow-md transition-all">
          ...
          <div>
            ...
            <h3 className="text-xl font-extrabold text-stone-800 mt-0.5">{stats.totalOrders}</h3>
          </div>
        </div>
        ...
        <div className="bg-white/85 backdrop-blur-xl p-5 rounded-[2rem] border border-[#EBE5DC] shadow-[0_8px_20px_rgba(0,0,0,0.02)] flex items-center gap-4 hover:shadow-md transition-all">
          ...
          <div>
            <p className="text-xs font-medium text-stone-400">Avg Order Value</p>
            <h3 className="text-lg font-extrabold text-stone-800 mt-0.5">฿{numberFormat(stats.avgOrderValue)}</h3>
          </div>
        </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div className="lg:col-span-2 bg-white/85 backdrop-blur-xl p-6 rounded-[2rem] border border-[#EBE5DC] shadow-[0_10px_30px_rgba(0,0,0,0.02)] space-y-4">
          <div>
            <p className="text-xs font-medium text-stone-400">ยอดขายการค้าทั้งหมด</p>
            <h2 className="text-2xl font-extrabold text-stone-800 mt-0.5">฿{numberFormat(stats.totalSales)}</h2>
          </div>
          ...
          <div className="h-52 w-full bg-[#FAF9F6] border border-[#EBE5DC]/60 rounded-[1.5rem] p-3">
            <ResponsiveContainer width="100%" height="100%">
              <LineChart data={chartData}>
                ...
                <Tooltip 
                  contentStyle={{ backgroundColor: '#ffffff', borderRadius: '16px', border: '1px solid #EBE5DC', boxShadow: '0 4px 20px rgba(0,0,0,0.05)' }}
                />
                <Line type="monotone" dataKey="sales" stroke="#D4A373" strokeWidth={3} dot={{ r: 5, fill: '#D4A373' }} activeDot={{ r: 7 }} />
              </LineChart>
            </ResponsiveContainer>
          </div>
        </div>
        ...
        <div className="bg-white/85 backdrop-blur-xl p-6 rounded-[2rem] border border-[#EBE5DC] shadow-[0_10px_30px_rgba(0,0,0,0.02)] flex flex-col justify-between">
          <h3 className="text-xs font-bold text-stone-700 uppercase tracking-wider">สถานะคำสั่งซื้อ</h3>
          ...
          <div className="flex items-center justify-center my-2">
            <div className="w-32 h-32 rounded-full border-8 border-emerald-400 border-t-amber-300 flex items-center justify-center shadow-inner bg-[#FAF9F6]">
              <span className="text-xs font-bold text-stone-600">Orders Ratio</span>
            </div>
          </div>

          <div className="space-y-2.5 text-xs">
            <div className="flex justify-between items-center text-stone-600 bg-[#FAF9F6] px-3 py-2 rounded-xl">
              <span className="flex items-center gap-2 font-medium"><span className="w-2.5 h-2.5 rounded-full bg-amber-400"></span> Not Process</span>
              <span className="font-bold">{stats.notProcessPercent}%</span>
            </div>
            <div className="flex justify-between items-center text-stone-600 bg-[#FAF9F6] px-3 py-2 rounded-xl">
              <span className="flex items-center gap-2 font-medium"><span className="w-2.5 h-2.5 rounded-full bg-emerald-400"></span> Completed</span>
      ...
      <div className="bg-white/85 backdrop-blur-xl p-6 sm:p-7 rounded-[2rem] border border-[#EBE5DC] shadow-[0_10px_30px_rgba(0,0,0,0.02)] space-y-4">
        <div className="flex items-center justify-between">
          <h3 className="text-xs font-bold text-stone-700 uppercase tracking-wider">Recent Orders</h3>
          <span className="text-xs text-stone-400 font-medium">รายการล่าสุดทั้งหมด {orders.length} รายการ</span>
        </div>
        ...
        <div className="overflow-x-auto">
          <table className="w-full text-left border-collapse">
            ...
            <tbody className="text-xs text-stone-700 divide-y divide-[#FAF9F6]">
              {orders.length > 0 ? (
                orders.map((ord, idx) => (
                  <tr key={idx} className="hover:bg-[#FAF9F6]/60 transition-all">
                    ...
                    <td className="p-3.5">
                      <span className={`px-3 py-1 rounded-xl font-semibold inline-block ${
